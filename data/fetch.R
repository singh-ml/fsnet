...
list_somaticMutation <- ProcessSomaticMutationData(inputFilePath = path_somaticMutation[1],
						   outputFileName = paste(sCancer,"somaticMutation", sep = "__"),
...
list_methylation_450 <- ProcessMethylation450Data(inputFilePath = path_methylation_450[1],
...
list_protein_RPPA <- ProcessRPPADataWithGeneAnnotation(inputFilePath = path_protein_RPPA[1],
						       outputFileName = paste(sCancer, "protein_RPPA", sep = "__"),
						       outputFileFolder = sPath2)
...
list_methylation_450_TSS <- CalculateSingleValueMethylationData(input = list_methylation_450,
...
write.table(cbind(list_CombinedData$Des, list_CombinedData$Data),
	    file = paste(sPath3, "CombinedMultiPlatformData.txt", sep = "/"),
	    quote = FALSE,
	    sep = "\t",
	    na = "",
	    col.names = TRUE,
